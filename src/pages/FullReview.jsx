...
const ELLE_RESPONSE = "I've reviewed your wedding contract and found several clauses worth your attention. The unilateral termination clause in Section 3 means the photographer can cancel at any time without penalty. I'd recommend asking them to add a mutual termination clause before signing."
...
export default function FullReview() {
  ...
  // step: 'default' | 'email-gate' | 'elle-response'
  ...
  const queryCount = submittedQuery ? 0 : 1
  const queryCountLabel = queryCount === 1 ? '1 query' : '0 queries'
  ...
  return (
    <div className={mkt.page}>
      ...
      <main>
        ...
        {/* ── ELLE WIDGET — full review + chatbox ── */}
        <section className={styles.widgetSection}>
          <div className={styles.widgetInner}>
            ...
            <div className={styles.widgetCols}>
              ...
              {/* Right column — Elle output, all issues unlocked, chatbox pinned bottom */}
              <div className={`${styles.elleCol} ${minimized ? styles.elleColMinimized : ''}`}>

                {/* Scrollable issues card */}
                ...
                {/* Chatbox — pinned to bottom of right column content area */}
                <div className={`${styles.chatbox} ${minimized ? styles.chatboxMinimized : ''}`}>
                  <div className={styles.chatboxHeader}>
                    <span className={styles.chatboxTitle}>Chat with Elle</span>
                    <button
                      type="button"
                      className={styles.chatboxToggle}
                      onClick={() => setMinimized(m => !m)}
                      aria-label={minimized ? 'Expand chatbox' : 'Minimize chatbox'}
                      aria-expanded={!minimized}
                    >
                      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d={minimized ? 'M4 6l4 4 4-4' : 'M4 10l4-4 4 4'} stroke="var(--color-text-primary)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </button>
                  </div>
                  ...
                  <div className={styles.chatboxBody}>
                    <div className={`${styles.chatboxContent} ${showEmailGate ? styles.chatboxContentBlurred : ''}`} aria-hidden={showEmailGate}>
                    <div className={styles.chatboxMessages} ref={messagesRef}>
                      <div className={styles.elleBubble}>
                        Hi! I can help you review and understand your document. Ask me about specific clauses, rights, or anything you're unsure about!
                      </div>
                    ...
                    <form className={styles.chatboxInputRow} onSubmit={handleQuerySubmit}>
                      <input
                        type="text"
                        ...
                        tabIndex={showEmailGate ? -1 : undefined}
                      />
                      <button
                        type="submit"
                        className={styles.chatboxSubmit}
                        aria-label="Submit question"
                        disabled={!!submittedQuery || !query.trim()}
                        tabIndex={showEmailGate ? -1 : undefined}
                      >
                    ...
                    <p className={styles.chatboxAccess}>
                      Free Access: You have <strong>{queryCountLabel}</strong> available.
                      {showElleResponse && (
                        <> <button type="button" className={styles.inlineLink} onClick={scrollToPricing}>Subscribe to a plan</button> to get unlimited queries.</>
                      )}
                    </p>
                    <p className={styles.chatboxFinePrint}>
                      Your documents are securely encrypted. This answer was generated by AI and is for informational purposes only. It does not constitute legal advice.
                    </p>
                    </div>

                    {showEmailGate && (
                      <div className={styles.emailGateOverlay}>
                      <div className={styles.emailGateCard}>
                        <h3 className={styles.emailGateHeading}>Elle's got your answer.</h3>
                        <p className={styles.emailGateSubtext}>
                          Enter your email and Elle will walk you through what this means for you.
                        </p>
                        <form className={styles.chatboxInputRow} onSubmit={handleEmailSubmit} noValidate>
                          <input
                            type="email"
                            className={`${styles.chatboxInput} ${emailError ? styles.chatboxInputError : ''}`}
                        ...
                        {emailError && (
                          <p className={styles.emailGateError}>
                            <span aria-hidden="true">ⓘ</span> {emailError}
                          </p>
                        )}
                        <p className={styles.emailGateFinePrint}>
                          By signing up, you agree to receive emails from LegalShield. View our <a href="#" className={styles.inlineLink}>Privacy Policy</a>
                        </p>
                      </div>
                    </div>
                    )}
