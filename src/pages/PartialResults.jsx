...
import styles from './PartialResults.module.css'
  ...
  return (
    <div className={mkt.page}>
      ...
      <main>
        ...
        {/* ── ELLE WIDGET — partial results ── */}
        <section className={styles.widgetSection}>
          <div className={styles.widgetInner}>

            {/* Eyebrow + heading above columns */}
            ...
            {/* Two-column layout — columns touch edge to edge */}
            <div className={styles.widgetCols}>
              ...
              {/* Right column — light purple background, relative for email gate pinning */}
              <div className={styles.elleCol}>

                {/* Scrollable inner content card */}
                <div className={styles.elleCard}>

                  {/* Panel header — POWERED BY ELLE */}
                  ...
                  <div className={styles.elleBody}>
                    ...
                    {/* 5 blurred cards with progressive fade */}
                    <div className={styles.blurStack}>
                      {BLURRED_ISSUES.map((issue, i) => (
                        <div key={i} className={`${styles.issueCard} ${styles.issueCardBlurred}`}>
                          <div className={styles.issueTop}>
                            <span className={styles.issueHeadline}>{issue.headline}</span>
                            <span className={`${styles.issueTag} ${styles[`issueTag--${issue.tagVariant}`]}`}>
                              {issue.tag}
                ...
                {/* Email gate — pinned to bottom of right column */}
                <div className={styles.emailGate}>
                  <p className={styles.emailGateHeadline}>Elle found 5 more issues worth reviewing</p>
                  <p className={styles.emailGateSubtext}>Enter your email to unlock the full review.</p>
                  <form className={styles.emailForm} onSubmit={handleSubmit} noValidate>
                    <div className={styles.emailInputRow}>
                      <input
                        type="email"
                        className={`${styles.emailInput} ${status === 'error' ? styles.emailInputError : ''} ${status === 'success' ? styles.emailInputSuccess : ''}`}
                        placeholder="Enter your email"
                        value={email}
                        onChange={e => { setEmail(e.target.value); setStatus('idle') }}
                      />
                      <button type="submit" className={styles.emailSubmitBtn} aria-label="Submit">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {status === 'success' && (
                      <p className={styles.successMsg}>✓ Got it! Unlocking your full review...</p>
                    )}
                  </form>
                  <p className={styles.emailFinePrint}>
